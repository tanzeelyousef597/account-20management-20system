...
export default function RejectedOrders({ 
  ...
  // Filter rejected orders from workers
  const rejectedOrders = useMemo(() => {
    let filtered = workOrders.filter(order => 
      order.createdBy !== 'admin' && order.status === 'Rejected'
    );
  ...
  const paginatedOrders = rejectedOrders.slice(startIndex, startIndex + itemsPerPage);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-4">
          <div className="relative">
            ...
            <Input
              placeholder="Search rejected orders by title, description, category, or worker..."
              value={searchQuery}
      ...
      <Card className="bg-gradient-to-br from-red-50 to-rose-50 border-red-200">
        <CardContent className="p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-red-600 font-medium">Rejected Orders</p>
              <p className="text-2xl font-bold text-red-800">{rejectedOrders.length}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Worker submissions that have been rejected
          </CardDescription>
        </CardHeader>
        <CardContent>
          {paginatedOrders.length > 0 ? (
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-500">
                    Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, rejectedOrders.length)} of {rejectedOrders.length} entries
                  </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">Rejected worker submissions will appear here</p>
            </div>
          )}
        </CardContent>
      </Card>
