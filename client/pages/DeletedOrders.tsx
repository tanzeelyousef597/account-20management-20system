...
export default function DeletedOrders({ 
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'Deleted': return <Trash2 className="h-4 w-4 text-gray-600" />;
      default: return <Trash2 className="h-4 w-4 text-gray-600" />;
    }
  };

  // Filter deleted orders from workers
  const deletedOrders = useMemo(() => {
    let filtered = workOrders.filter(order => 
      order.createdBy !== 'admin' && order.status === 'Deleted'
    );
  ...
  // Pagination logic
  const totalPages = Math.ceil(deletedOrders.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedOrders = deletedOrders.slice(startIndex, startIndex + itemsPerPage);

  const goToPage = (page: number) => {
    setCurrentPage(Math.max(1, Math.min(page, totalPages)));
  };

  const handleRestore = async (orderId: string, newStatus: string) => {
    if (confirm(`Are you sure you want to restore this order and move it to ${newStatus}?`)) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-4">
          <div className="relative">
            ...
            <Input
              placeholder="Search deleted orders by title, description, category, or worker..."
              value={searchQuery}
      ...
      <Card className="bg-gradient-to-br from-gray-50 to-slate-50 border-gray-200">
        <CardContent className="p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-gray-600 font-medium">Deleted Orders</p>
              <p className="text-2xl font-bold text-gray-800">{deletedOrders.length}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Worker submissions that have been deleted (can be restored)
          </CardDescription>
        </CardHeader>
        <CardContent>
          {paginatedOrders.length > 0 ? (
                ...
                <TableBody>
                  {paginatedOrders.map((order) => (
                    <TableRow key={order.id} className="opacity-75">
                      ...
                      <TableCell>
                        <Badge variant="outline" className="text-gray-600">{order.category}</Badge>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => handleRestore(order.id, 'Approved')}>
                              <CheckCircle className="h-4 w-4 mr-2" />
                              Restore to Approved
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleRestore(order.id, 'Rejected')}>
                              <XCircle className="h-4 w-4 mr-2" />
                              Restore to Rejected
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleRestore(order.id, 'Under QA')}>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-500">
                    Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, deletedOrders.length)} of {deletedOrders.length} entries
                  </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">Deleted worker submissions will appear here</p>
            </div>
          )}
        </CardContent>
      </Card>
